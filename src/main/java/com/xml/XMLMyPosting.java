...
public class XMLMyPosting extends AsyncTask<Object, String, Long> {
	...
	private MyPostingListener activityCallback;
	...
	public XMLMyPosting(MyPostingListener activityCallback, Activity activity) {
	...
	protected void onPostExecute(Long result) {
		...
		if (isHasConnectionErrorOccurred() || isHasParsingErrorOccurred()) {
			...
			if (bookList != null || bookList.size() != 0) {
				if (!bookList.get(0).getUniversityName().equals("fail")) {

					getActivityCallback().dataLoadedBook(bookList);
				} else
					Toast.makeText(getActivity().getApplicationContext(),
	...
	private MyPostingListener getActivityCallback() {
		return activityCallback;
	}

	private void setActivityCallback(MyPostingListener activityCallback2) {
